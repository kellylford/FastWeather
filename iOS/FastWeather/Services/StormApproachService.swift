...
final class StormApproachService {
    ...
    /// - Parameter displayedConditionCode: the WMO code the app is currently showing for this
    ///   city (post-WeatherKit-overlay). Used for the dry-convection reconciliation note so it
    ///   agrees with the condition the user sees, not the raw Open-Meteo model code.
    func fetchStormApproach(for city: City,
                            nearbySavedCities: [City] = [],
                            displayedConditionCode: Int? = nil) async throws -> StormApproach {
    ...
    private func analyse(city: City,
        ...
        // Convective awareness: thunderstorm condition at the location with no measurable
        // precipitation (scattered/dry convection). The note exists to AGREE with the condition
        // the main screen shows, so it must key off the same source. With the WeatherKit
        // conditions overlay on, the main screen no longer shows Open-Meteo's phantom
        // thunderstorm codes — so when the caller can't supply the displayed code, suppress
        // the note rather than reconcile against a condition the user never sees.
        let centreCode: Int
        if let displayed = displayedConditionCode {
            centreCode = displayed
        } else if FeatureFlags.shared.weatherKitConditionsEnabled {
            centreCode = -1
        } else {
            centreCode = sampleForecasts.first?.current?.weatherCode ?? -1
        }
